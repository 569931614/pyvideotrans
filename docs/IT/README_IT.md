...
>
> La sintesi vocale del testo supporta `Microsoft Edge tts`, `Google tts`, `Azure AI TTS`, `Openai TTS`, `Elevenlabs TTS`, `Custom TTS server api`, `GPT-SoVITS`, `clone-voice`, `ChatTTS-ui`, `CosyVoice`
>
...
0. Scarica la versione preconfezionata, estrai in una directory in inglese senza spazi, dopodiché fai doppio clic su sp.exe
...
2. Esegui il comando `cd BDvideoTrans`
...
[Dettagliata soluzione di distribuzione per Mac](https://BDvideoTrans.com/mac.html)
...
2. Continua con il comando `cd BDvideoTrans`
3. Continua con `python -m venv venv`
4. Continua con il comando `source ./venv/bin/activate`, dopo aver finito controlla di confermare che la richiesta del terminale sia iniziata con `(venv)`, i comandi successivi devono essere eseguiti solo dopo aver confermato che la richiesta del terminale inizia con `(venv)`
5. Esegui `pip install -r requirements.txt `, se fallisce, esegui i seguenti due comandi per cambiare lo specchio pip in quello di Aliyun
...
6. Se vuoi usare l'accelerazione CUDA, esegui separatamente
...
1. Trova una cartella senza spazi e caratteri cinesi, scrivi `cmd` nella barra degli indirizzi e premi invio per aprire la console, i comandi successivi dovrebbero essere eseguiti in questa console
2. Continua con il comando `cd BDvideoTrans`
3. Continua con `python -m venv venv`
4. Continua con il comando `.\venv\scripts\activate`, dopo verifica che la richiesta del terminale sia iniziata con `(venv)`, altrimenti c'è un errore
5. Esegui `pip install -r requirements.txt `, se fallisce, esegui i seguenti due comandi per cambiare lo specchio pip in quello di Aliyun
...
7. Su windows se vuoi attivare l'accelerazione cuda, devi avere una scheda grafica Nvidia e un ambiente CUDA11.8+ configurato, vedi [Supporto accelerazione CUDA](https://BDvideoTrans.com/gpu.html)

8. Estrai ffmpeg.zip nella directory corrente del codice sorgente, se richiesto sovrascrivi, dopo assicurati di trovare ffmpeg.exe ffprobe.exe ytwin32.exe nella cartella ffmpeg del codice sorgente,
...
Si prega di controllare https://BDvideoTrans.com/guide.html


# Modelli di riconoscimento vocale:

   Indirizzo di download: https://BDvideoTrans.com/model.html

   Spiegazione e differenze dei modelli: https://BDvideoTrans.com/02.html
